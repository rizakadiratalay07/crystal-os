...
INVALID_STOCK_WORDS = {
    ...
    "HİSSE", "SENET", "ANALİZ", "TEKNİK", "TEMEL", "GRAFİK"
}
...
def is_market_query(text):
    keywords = [
        "piyasa", "borsa", "endeks", "xu100", "xu030", "xutum", "bist",
        "bist100", "bist30", "bist50", "bugün piyasalar", "piyasalar nasıl",
        "borsa nasıl", "endeks nasıl", "dolar", "euro"
    ]
...
class InteractiveChartCanvas(FigureCanvas):
    ...
    def _throttled_draw(self, force=False):
        ...
        if not force and self._last_draw_ms:
            ...
            if delta < 0:
                delta += 24 * 60 * 60 * 1000
            if delta < self._draw_interval_ms:
                return
...
class MarketChartWidget(QtWidgets.QWidget):
    ...
    def _update_visible_xticks(self):
        if self._chart_df.empty or len(self._axes) < 2:
            return
...
def _fix_yahoo_data(data):
    ...
    if not all(col in data.columns for col in required[:4]):
        return None
    return data[required].sort_index()

def get_data_yahoo(symbol, period='max'):
    try:
        kw = {'interval': '1d', 'period': period, 'auto_adjust': False, 'progress': False, 'threads': False}
        data = yf.download(symbol, **kw)
        if data is None or data.empty:
            print(f"⚠️ Yahoo veri yok: {symbol}")
            return None
        data = _fix_yahoo_data(data)
        if data is None or data.empty:
            print(f"⚠️ Yahoo kolonları geçersiz: {symbol}")
            return None
...
def get_technical(symbol, period='max'):
...
def format_technical_data_for_prompt(data):
    ...
    history_str = ""
    hist = data.get("_chart_df")
    if hist is not None and not hist.empty:
        df = hist.copy()
        sq_series = data.get("sqzmom_series")
        if sq_series is None:
            sq_series = sqzmom(df)["series"]
        recent = df.tail(MAX_ROWS)
        recent_sq = sq_series.reindex(recent.index)
        rows = []
        for idx, row in recent.iterrows():
            date_str = idx.strftime("%Y-%m-%d") if hasattr(idx, 'strftime') else str(idx)
            close = row["Close"]
            volume = row["Volume"]
            dc_basis = row.get("DC_basis", np.nan)
            bb_middle = row.get("BB_middle", np.nan)
            sq_val = recent_sq.loc[idx] if idx in recent_sq.index else np.nan
            rows.append(f"{date_str}: Kapanış={close:.2f}, Hacim={volume:.0f}, DC_basis={dc_basis:.2f}, BB_orta={bb_middle:.2f}, SQZMOM={sq_val:.3f}")
        history_str = "\n".join(rows)
...
Son {MAX_ROWS} Günün Günlük Verileri:
{history_str}
"""

def clean_report(report):
    for marker in ["Başka:", "Başka :", "BAŞKA:"]:
...
def ask_llm_analysis(data):
    prompt = f"""
Sen profesyonel bir Türk teknik analiz asistanısın. Yanıtını YALNIZCA Türkçe yaz. Türkçe dışında hiçbir kelime veya karakter kullanma (İngilizce, Çince, Arapça vb. kesinlikle yasak).
Verilen verilerden yola çıkarak Türkçe, kapsamlı ama net bir analiz yap. Yatırım tavsiyesi verme.
Aşağıda güncel özet ve son {MAX_ROWS} günün verileri var. Geçmiş sinyallerle karşılaştırma yap ve önemli bulguları vurgula.
Yanıtında aşağıdaki başlıkları aynen kullan ve her başlık altına 2-3 cümle yaz (toplam 15-18 cümle civarı olsun):
Heiken-Ashi & DC (Donchian Kanalları):
SQZMOM (Sıkışma İvmesi):
Trend:
Support/Resistance (Destek/Direnç):
Olası Alım:
Olası Satım:
Stoploss (Kayıp Durdur):
Özet:
SADECE bu başlıkları kullan. "Başka" adında bir başlık KESİNLİKLE kullanma. "Özet" bölümünü de en az 2-3 cümleyle tamamla, yarım bırakma. Başka başlık ekleme. Başlıkları değiştirme veya başka dile çevirme. Seviyeleri belirt. Aynı şeyi tekrar etme.
...
system = "Sen profesyonel bir Türk borsa analistisin. Yanıtların Türkçe, objektif, net ve yeterince kapsamlı olmalı. Yatırım tavsiyesi verme. Kesinlikle Türkçe dışında bir dil kullanma. Yabancı kelime yazma. Belirtilen başlık dışında başlık kullanma."
...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def append_system(self, text):
        safe = text.replace("\n", "<br>")
        self.chat.append(f'<p style="margin:0 0 12px 0; color:#ff9800;"><b>PUSULA AI</b><br><span style="color:#aaaaaa;">{safe}</span></p>')

    def append_user(self, text):
        safe = (text.replace("&", "&amp;").replace("<", "&lt;").replace(">", "&gt;").replace("\n", "<br>"))
        self.chat.append(f'<p style="margin:0 0 12px 0; color:#1976d2;"><b>SİZ</b><br><span style="color:#20242b;">{safe}</span></p>')

    def append_ai(self, text):
        safe = (str(text).replace("&", "&amp;").replace("<", "&lt;").replace(">", "&gt;").replace("\n", "<br>"))
        self.chat.append(f'<p style="margin:0 0 12px 0; color:#2e7d32;"><b>PUSULA AI</b><br><span style="color:#20242b;">{safe}</span></p>')
    ...
    def set_finance_mode(self, chart_df, symbol):
        ...
        if chart_df is not None:
            self.chart_panel.plot(chart_df, symbol)
    ...
    def send_message(self):
        ...
        if is_market_query(text):
            ...
            self.chat.append('<p style="margin:0 0 12px 0; color:#ff9800;"><b>🔎 XU100 analizi yapılıyor...</b></p>')
        ...
        if sym:
            ...
            self.chat.append(f'<p style="margin:0 0 12px 0; color:#ff9800;"><b>🔎 {sym} analizi yapılıyor...</b></p>')
